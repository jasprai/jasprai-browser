...
// Template data (empty to avoid pre-populating user prompts)
const defaultFavoritePrompts: Array<{ title: string; content: string }> = [];
...
export function createFavoritesStorage(): FavoritePromptsStorage {
  return {
    ...
    getAllPrompts: async (): Promise<FavoritePrompt[]> => {
      ...
      // Do not auto-seed prompts to avoid exposing defaults
